...
post_button = document.getElementsByClassName('post_button');
...
function show_post_box(){
    ...
    if(post_box.style.display === 'none'){
       ...
       post_box.style.opacity = "1 !important"
        ...
        create_post_divbuttn.firstElementChild.innerText = 'Cancel post';
            ...
            create_post_divbuttn.firstElementChild.innerText = 'Create post';
...
async function show_all_posts(){
  ...
  Xhr.onload = () => {
    ...
    break_el = document.createElement('br');
          div.innerText = `${data[index].name} \n\n\n ${data[index].content}`
            all_posts_div.appendChild(div)
            all_posts_div.appendChild(break_el)
